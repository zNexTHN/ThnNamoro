...
export function RelationshipUI({ onClose }: RelationshipUIProps) {
  ...
  const handleAcceptRequest = async () => {
    if (inFiveM) {
      ...
      if (devState.pendingRequest) {
        const newStatus = devState.pendingRequest.type === 'marriage' 
          ? 'married' 
          : devState.pendingRequest.type === 'engagement' 
          ? 'engaged' 
          : 'dating';
        setDevState({
          status: newStatus,
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center p-4">
      {/* Main Container - Oculta se o modal de requisição estiver ativo */}
      {!showRequestModal && (
        <motion.div
