...
export function RelationshipUI({ onClose }: RelationshipUIProps) {
  ...
  const state: IRelationshipState = inFiveM && relationshipData 
    ? {
        status: relationshipData.status === 'single' ? 'single' : 
                relationshipData.status === 'dating' ? 'dating' : 'married',
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center p-4">
      {/* Main Container - SÓ MOSTRA SE NÃO TIVER PEDIDO (!showRequestModal) */}
      {!showRequestModal && (
        <motion.div
        ...
        >
          ...
          <div className="relative z-10 flex-1 overflow-y-auto">
            <AnimatePresence mode="wait">
              ...
              {(state.status === 'dating' || state.status === 'married') && state.partner && (
                <RelationshipState
                  key="relationship"
                  status={state.status}
      ...
      {/* Request Modal - MOSTRA QUANDO TIVER PEDIDO */}
      <AnimatePresence>
